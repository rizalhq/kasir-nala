        ...
        <table class="table" id="dataTable">
            ...
            <tbody class="table-border-bottom-0" id="tableBody">
                ...
                <tr>
                    ...
                    <td><?= $row['jumlah_barang'] ?> Meter</td>
                    <td>
                        <a onclick="return confirm('Apakah anda yakin menghapus data ini?')"
                            href="<?= base_url('produk/hapus/'.$row['id_produk']) ?>"
                            class="btn btn-danger btn-sm text-white">Hapus</a>
                        <button type="button" class="btn btn-warning btn-sm" data-bs-toggle="modal"
                            data-bs-target="#gantiproduk-<?= $row['id_produk'] ?>">
                            Edit Produk
                        </button>
                    </td>
                </tr>
                <!-- edit -->
                <div class="modal fade" id="gantiproduk-<?= $row['id_produk'] ?>" tabindex="-1" aria-labelledby="gantiprodukLabel" aria-hidden="true">
                	<div class="modal-dialog">
                		<form action="<?= base_url('produk/update/' . $row['id_produk']) ?>" method="post">
                			<div class="modal-content">
                				<div class="modal-header">
                					<h5 class="modal-title" id="gantiproduk">Edit Data untuk <?= $row['nama_produk'] ?></h5>
                					<button type="button" class="btn-close" data-bs-dismiss="modal"
                						aria-label="Close"></button>
                				</div>
                				<div class="modal-body">
                					<div class="mb-3">
                						<label for="nama-<?= $row['id_produk'] ?>" class="form-label">Nama</label>
